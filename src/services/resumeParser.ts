import { ParsedResume, ResumeAnalysisResult } from "../types";
import { DEMO_RESUME_ANALYSIS } from "../lib/demoData";
...
export async function parseResumeText(rawText: string): Promise<ResumeAnalysisResult> {
  ...
  // If text is short or demo, return high quality demo analysis
  if (!trimmed || trimmed.length < 50 || trimmed.toLowerCase().includes("alex sharma")) {
