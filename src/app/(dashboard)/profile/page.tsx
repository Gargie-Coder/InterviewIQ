...
import { User, CheckCircle2, Save, Mail, Briefcase, GraduationCap, Sparkles } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const [targetRole, setTargetRole] = useState<RoleType>(user.targetRole);
  const [experienceYears, setExperienceYears] = useState(user.experienceYears);
  const [education, setEducation] = useState(user.education);
  const [skillsInput, setSkillsInput] = useState(user.skills.join(", "));
  const [interviewType, setInterviewType] = useState(user.preferredInterviewType);
      ...
      <Card>
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSave} className="space-y-5">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Preferred Mock Interview Question Archetype
              </label>
            ...
            <div className="pt-2 flex justify-end">
              <Button type="submit" variant="primary" size="md" className="font-semibold shadow-subtle">
